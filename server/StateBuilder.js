...
export default async function getState(
  ...
  } else if (PUBLIC_API[routeParts[0]] || (route.page === 'PostsIndex' && route.params.username)) {
...
async function getStateForProfile(state, route, { api }) {
  ...
  switch (route.params.category) {
    case 'recent-replies':
      const replies = await api.getRepliesByLastUpdateAsync(username, '', 50, DEFAULT_VOTE_LIMIT);
