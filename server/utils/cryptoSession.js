...
module.exports = function(app, opts) {
  ...
  try {
    ...
    if (secret.length !== sodium.crypto_secretbox_KEYBYTES)
      throw new Error(
        `Crypto key should decode to ${sodium.crypto_secretbox_KEYBYTES} bytes in length`
      );
  } catch (error) {
  ...
  function encode(body) {
    try {
      ...
      console.error(
        'Crypto-session: encode error resetting session',
        body,
        err,
  ...
  function decode(text) {
    try {
      if (!/^crypto-session:/.test(text)) throw 'Unrecognized encrypted session format.';
      ...
      console.error(
        'Crypto-session: decode error resetting session',
        text,
        err,
        err ? err.stack : undefined
      );
