...
function prod_logger() {
  return function* logger(next) {
    ...
    if (!asset) {
      ...
      console.log(
        `[reqid ${this.request.header['x-request-id']}] ${uid}${this.method} ${this.originalUrl}`
      );
    }
    try {
      yield next;
...
function log(ctx, start, len, err, asset) {
  ...
  if (!asset || err || ctx.status > 400) {
    console.log(
      `${upstream}[reqid ${ctx.request.header['x-request-id']}] ${ctx.session.uid || ''} ${
        ctx.method
      } ${ctx.originalUrl} ${status} ${time(start)} ${length}`
    );
  }
