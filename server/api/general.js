...
import { api, broadcast } from 'mocks/golos-js';
...
export default function useGeneralApi(app) {
    ...
    router.post('/accounts', koaBody, function*() {
        ...
        try {
            const lock_entity_res = yield Tarantool.instance('tarantool').call(
                'lock_entity',
                user_id + ''
            );
            ...
            yield new Promise(resolve => setTimeout(() => resolve(), rnd_wait_time));
        }
        ...
        try {
            ...
            if (same_ip_bot) {
                ...
                this.body = JSON.stringify({
                    error:
                        'We are sorry, we cannot sign you up at this time because your IP address is associated with bots activity. Please contact t@cyber.fund for more information.',
                });
                this.status = 401;
                return;
            }
            ...
            if (existing_account) {
                throw new Error(
                    'Only one Golos account per user is allowed in order to prevent abuse'
                );
            }
            ...
            if (same_ip_account) {
                ...
                if (minutes < 10) {
                    ...
                    throw new Error(
                        'Only one Golos account allowed per IP address every 10 minutes'
                    );
                }
            }
            ...
            if (!mid) {
                ...
                console.log(
                    `api /accounts: is confirmed sms for user ${this.session.uid} #${user_id}`
                );
            }
            ...
            try {
                ...
                if (chain_fee && chain_fee > fee) {
                    if (fee / chain_fee > 0.5) {
                        // just a sanity check - chain fee shouldn't be a way larger
                        console.log(
                            '-- /accounts warning: chain_fee is larger than config fee -->',
                            this.session.uid,
                            fee,
                            chain_fee
                        );
            ...
            const accountInstance = yield models.Account.create(
                escAttrs({
                    user_id,
                    name: account.name,
            ...
            try {
                yield Tarantool.instance('tarantool').call('unlock_entity', user_id + '');
            } catch (e) {
                /* ram lock */
            }
        }
