...
class PostSummary extends Component {
    ...
    render() {
        ...
        if (reblogged_by) {
            reblogged_by = (
                <div className="PostSummary__reblogged_by">
                    <Icon name="reblog" /> {tt('postsummary_jsx.resteemed_by')}{' '}
        ...
        if (isNsfw && p.json_metadata && p.json_metadata.tags)
            currentNsfw = p.json_metadata.tags.filter(function(n) {
                return nsfwTags.indexOf(n) >= 0;
            });
        if (currentNsfw && currentNsfw.length) nsfwTitle = currentNsfw[0];
        let desc = p.desc;
        if (p.image_link)
            // image link is already shown in the preview
        ...
        return (
            <article
                className={
                    'PostSummary hentry' + (thumb ? ' with-image ' : ' ') + commentClasses.join(' ')
                }
                itemScope
            ...
            >
                ...
                {reblogged_by}
                <div className="PostSummary__header show-for-small-only">{content_title}</div>
                <div className="PostSummary__time_author_category_small show-for-small-only">
                    {author_category}
                </div>
                {thumb}
                <div className="PostSummary__content">
                    <div className={'PostSummary__header show-for-medium'}>{content_title}</div>
                    {content_body}
                    {content_footer}
                </div>
            </article>
