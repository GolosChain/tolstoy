...
export default class Online extends PureComponent {
  ...
  componentDidMount() {
    this.setState({
      data: {
        ...this.state.data,
        switchAll: Object.values(this.state.data.notify.show).every(value => value === true),
      },
    });
  }
  ...
  render() {
    ...
    return (
      <Form onSubmit={this.handleSubmit} decorators={[calculateSwitchAll]} initialValues={data}>
                ...
                {({ input }) => (
                  <FormGroup>
                    ...
                    <FormRow>
                      ...
                      <OptionText dark>{tt('settings_jsx.notifications.allOnline')}</OptionText>
                      <Switcher {...input} />
                    </FormRow>
                  </FormGroup>
                )}
