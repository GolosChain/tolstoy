...
import { DEBT_TOKEN_SHORT, LIQUID_TICKER, CURRENCY_SIGN } from 'src/constants/config';
...
function generateDepthChart(bidsArray, asksArray) {
  ...
  let depth_chart_config = {
    ...
    tooltip: {
      ...
      formatter() {
        return (
          `<span>${tt('g.price')}: ${(this.x / power).toFixed(6)} ${CURRENCY_SIGN}/${tt(
            'token_names.LIQUID_TOKEN_UPPERCASE'
          )}</span><br/><span>\u25CF</span>${this.series.name}: <b>${(this.y / 1000).toFixed(
            3
          )} ${DEBT_TOKEN_SHORT} ` +
          '(' +
          CURRENCY_SIGN +
