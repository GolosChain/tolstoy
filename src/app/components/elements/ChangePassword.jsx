...
import { validate_account_name } from 'src/app/utils/ChainValidation';
import { cleanReduxInput } from 'src/app/utils/ReduxForms';
...
class ChangePassword extends React.Component {
  ...
  dispatchSubmit = () => {
    ...
    const success = () => {
      ...
      window.location = `/login#account=${accountName}&msg=passwordupdated`;
    };
    const error = e => {
  ...
  render() {
    ...
    const error2 = /Missing Owner Authority/.test(error) ? (
      <span>
        ...
        <a href="/recover_account_step_1">{tt('g.recover_your_account')}</a>?
      </span>
    ) : (
      error
    );
        ...
        >
          ...
          <label>
            <div className="float-right">
              <a href="/recover_account_step_1">{tt('g.recover_password')}</a>
            </div>
          ...
          <label>
            {tt('g.generated_password') + ' '} <span className="secondary">({tt('g.new')})</span>
            <br />
          </label>
          {(generated && (
            <span>
            ...
            <center>
              <button type="button" className="button hollow" onClick={this.generateWif}>
          ...
          <label>
            <input {...cleanReduxInput(confirmCheck)} type="checkbox" />{' '}
            {tt('g.understand_that_APP_NAME_cannot_recover_password', {
              APP_NAME: 'GOLOS.io',
            })}
            .
          </label>
          {confirmCheck.touched && confirmCheck.error && (
            <div className="error">{confirmCheck.error}</div>
          )}

          <label>
            <input {...cleanReduxInput(confirmSaved)} type="checkbox" />
            {tt('g.i_saved_password')}.
          </label>
          {confirmSaved.touched && confirmSaved.error && (
