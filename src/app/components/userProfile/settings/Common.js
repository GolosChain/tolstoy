...
import { CURRENCIES, LANGUAGES, AUCTION_REWARD_DESTINATION } from 'src/app/constants/config';
              ...
              <Field name="basic.auctionRewardDestination">
                {({ input }) => (
                  <FormGroup>
                    ...
                    <Select {...input} onChange={e => input.onChange(e.target.value)}>
                      {Object.keys(AUCTION_REWARD_DESTINATION.DESTINATION).map(key => (
                        <option key={key} value={key}>
                          {tt(`settings_jsx.auction_reward_destination.items.${key}`)}
                        </option>
                      ))}
                    </Select>
                  </FormGroup>
