...
import remarkableStripper from 'src/app/utils/RemarkableStripper';
import { htmlDecode } from 'src/app/utils/Html';
...
function extractDescBody(data) {
  ...
  const body2 = remarkableStripper.render(depth > 1 ? body.replace(/>([\s\S]*?).*\s*/g, '') : body);
